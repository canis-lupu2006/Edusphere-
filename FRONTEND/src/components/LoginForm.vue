<template>
  <form class="glass-card p-8 shadow-[0_0_40px_rgba(59,130,246,0.12)]" @submit.prevent="submit">
    <div class="mb-6 text-center">
      <div
        class="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full border border-blue-400/40 bg-blue-500/15 shadow-glow"
      >
        <Lock class="h-6 w-6 text-blue-300" />
      </div>
      <h2 class="font-display text-xl font-semibold">Connexion sécurisée</h2>
      <p class="mt-1 text-sm text-white/45">
        Accès réservé aux utilisateurs EduSphere et EDU OS.
      </p>
    </div>

    <label class="mb-1.5 block text-sm text-white/70">Adresse e-mail</label>
    <div class="relative mb-4">
      <Mail class="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-white/35" />
      <input
        v-model="email"
        type="email"
        required
    ...
    <div class="mb-1.5 flex items-center justify-between">
      ...
      <button type="button" class="text-xs text-violet-300 hover:underline" @click="forgotHint = true">
        Mot de passe oublié ?
      </button>
    </div>
    <div class="relative mb-2">
      <Lock class="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-white/35" />
      <input
        v-model="password"
    ...
    <p v-if="forgotHint" class="mb-4 text-xs text-white/40">
      Contactez l'administration de votre établissement pour réinitialiser votre accès.
    </p>

    <p v-if="error" class="mb-4 rounded-lg bg-red-500/10 px-3 py-2 text-sm text-red-300">
      {{ error }}
    </p>
    ...
    <div class="my-5 flex items-center gap-3 text-xs text-white/30">
    ...
    <div class="mt-6 flex flex-wrap justify-center gap-4 text-[10px] text-white/40">
